colorlegend <- function(colbar, labels, at=NULL,  xlim=c(0,1), ylim=c(0,1), 
vertical=TRUE, ratio.colbar = 0.4, lim.segment = NULL, align=c("c","l","r"),
addlabels=TRUE, ...) {
...
if(vertical){
	...
	rect(rep(xlim[1], len),  yyy[1:len],
		rep(xlim[1] +xgap*rat1, len), yyy[-1], col = colbar, border = colbar)
	...
	if(addlabels){
		if(align=="l") text(x=pos.xlabel, y=at, labels=labels, pos = 4, ...)
		if(align=="c") text((pos.xlabel+xlim[2])/2, y=at, labels=labels, ...)
		if(align=="r") text(x=xlim[2], y=at, labels=labels, pos = 2, ...)
	}
}
...
if(!vertical){
	...
	segments(at , ylim[2] - ygap*rat2[1],
		at, ylim[2] - ygap*rat2[2])
	if(addlabels){
		if(align=="l") text(at, pos.ylabel, labels, pos = 1, ...)
		if(align=="c") text(at, (pos.ylabel+ylim[1])/2, labels=labels, ...)
		if(align=="r") text(at, ylim[1], labels=labels, pos = 2, ...)
	}
}	
